...
class User(UserMixin,db.Model):
    ...
    dob=db.Column(db.String(100000))
...
def signup():
    if request.method=="POST":
       ...
       # new_user=db.engine.execute(f"INSERT INTO `user` (`srfid`,`email`,`dob`) VALUES ('{srfid}','{email} ")
        myquery=User(srfid=srfid,email=email,dob=dob)
    ...
    if('user' in session and session['user']=="admin"):
        ...
        if request.method=="POST":
           ...
           # db.engine.execute(f"INSERT INTO `hospitaluser` (`hcode`,`email`,`password`) VALUES ('{hcode}','{email}','{encpassword}') ")
            myquery=Hospitaluser(hcode=hcode,email=email,password=encpassword)
            db.session.add(myquery)
            db.session.commit()
...
def addhospitalinfo():
    ...
    if request.method=="POST":
        ...
        if huser:            
         ...
         myquery=Hospitaluser(hcode=hcode,hname=hname,normalbed=nbed,hibed=hbed,icubed=ibed,vbed=vbed)
         db.session.add(myquery)
         db.session.commit()
         db.session.add(myquery)
         db.session.commit()
...
def hdelete(id):
   ...
   query=Hospitaldata.query.filter_by(id=id).first()
   ...
   # query=db.engine.execute(f"SELECT * FROM `hospitaldata` ")
    ...
    if request.method=="POST":
       ...
       # dbb=db.engine.execute(f"SELECT * FROM `hospitaldata` WHERE `hospitaldata`.`hcode`='{code}' ")   
        query=Hospitaldata().query.filter_by(hcode=code)     
        bedtype=bedtype
